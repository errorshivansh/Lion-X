...
@bot.on(Lion_cmd(pattern="autoname"))
async def _(event):
    if event.fwd_from:
        return
    Lion = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    while True:
        HB = time.strftime("%d-%m-%y")
        HE = time.strftime("%H:%M")
        name = f"🕒{HE} ⚡{LIONX_USER}⚡ 📅{HB}"
        logger.info(name)
        try:
            await bot(
        ...
        await Lion.edit(f"Auto Name has been started my Master")
        await bot.send_message(Config.LOGGER_ID, "#AUTONAME \n\nAutoname Started!!")


@bot.on(Lion_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(Lion_cmd(pattern="reserved", outgoing=True))
