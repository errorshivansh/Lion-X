...
@bot.on(Lion_cmd(pattern="paste ?(.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern="getpaste(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="getpaste(?: |$)(.*)", allow_sudo=True))
async def get_dogbin_content(dog_url):
    textx = await dog_url.get_reply_message()
    message = dog_url.pattern_match.group(1)
    Lion = await eor(dog_url, "`Getting dogbin content...`")
    ...
    if message.startswith(format_view):
        ...
        await eod(Lion, "`Is that even a dogbin url?`")
        return
    ...
    try:
        ...
        await eod(Lion, "Request returned an unsuccessful status code.\n\n" + str(HTTPErr)
        )
        return
    except exceptions.Timeout as TimeoutErr:
        await eod(Lion, "Request timed out." + str(TimeoutErr))
        return
    except exceptions.TooManyRedirects as RedirectsErr:
        await eod(Lion, "Request exceeded the configured number of maximum redirections."
            + str(RedirectsErr)
        )
        return
...
@bot.on(Lion_cmd(pattern="neko ?(.*)", outgoing=True))
