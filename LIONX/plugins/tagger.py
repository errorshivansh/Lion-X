...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        ammoca_message = ""
        crazy = await event.client.get_entity(event.sender_id)
        if crazy.bot or crazy.verified or crazy.support:
            return
        crazym = f"[{get_display_name(crazy)}](tg://user?id={crazy.id})"
...
@bot.on(Lion_cmd(pattern=r"tagall (.*)", outgoing=True))
