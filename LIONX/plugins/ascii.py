...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    bot = "@asciiart_bot"
    crazy = await eor(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(bot) as conv:
        try:
            ...
            await crazy.edit("User Blocked!! Please Unblock @asciiart_bot and try again...")
            return
        await crazy.delete()
...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    bot = "@lines50bot"
    crazy = await eor(event, "`Processing...`")
    async with event.client.conversation(bot) as conv:
        try:
            first = await conv.send_message("/start")
            response = await conv.get_response()
            ...
            await crazy.edit("User Blocked!! Please Unblock @Lines50Bot and try again...")
            return
        await crazy.delete()
        final = await event.client.send_file(
            event.chat_id,
