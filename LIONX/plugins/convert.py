...
@bot.on(Lion_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(Lion):
    if Lion.fwd_from:
        return
    reply_to_id = Lion.message.id
    if Lion.reply_to_msg_id:
        reply_to_id = Lion.reply_to_msg_id
    event = await eor(Lion, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Lion.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Lion.client.send_file(
                event.chat_id,
...
async def _(event):
    ...
    chat = "@tgstogifbot"
    Lionevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, Lionreply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await Lionevent.edit("`This file is not supported`")
            Lionresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            Lionfile = Path(await event.client.download_media(Lionresponse, "./temp/"))
            Liongif = Path(await unzip(Lionfile))
            crazy = await bot.send_file(
                event.chat_id,
                Liongif,
            ...
            await bot(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=crazy.media.document.id,
                        access_hash=crazy.media.document.access_hash,
                        file_reference=crazy.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            ...
            await Lionevent.edit("Unblock @tgstogifbot")
            return


@bot.on(Lion_cmd(pattern="nfc ?(.*)"))
