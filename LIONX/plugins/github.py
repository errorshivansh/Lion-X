...
@bot.on(Lion_cmd(pattern=r"commit"))
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await eod(event, "`Please ADD Proper Github Repo Name of Lion-X`")
        return
    ...
    try:
        ...
        await LIONX.edit("Committing to Github....")
        await git_commit(downloaded_file_name, LIONX)


async def git_commit(file_name, LIONX):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await LIONX.edit("`File Already Exists`")
            create_file = False
    file_name = "LIONX/plugins/" + file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(LIONX, "Cannot Upload Plugin")
    else:
        return await eod(LIONX, "`Committed Suicide`")


@bot.on(Lion_cmd(pattern="github (.*)", outgoing=True))
