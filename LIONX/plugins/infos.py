...
@bot.on(Lion_cmd(pattern="recognize ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Lion = await eor(event, "recognizeing this media")
...
@bot.on(Lion_cmd(pattern="info ?(.*)", outgoing=True))
...
<b>⚡ <a href='https://t.me/The_LionX'>From DataBase of Lion-X</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@bot.on(Lion_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
    if event.fwd_from:
        return
    Lion = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await Lion.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(Lion, "`An unexpected error has occurred.`")
    return
...
@bot.on(Lion_cmd(pattern=r"users ?(.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern="admins ?(.*)"))
...
@bot.on(Lion_cmd(pattern="id$"))
@bot.on(sudo_cmd(pattern="id$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lion = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await Lion.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
