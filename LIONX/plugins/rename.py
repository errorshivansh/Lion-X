...
@bot.on(Lion_cmd(pattern="rename (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="rename (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lion = await eor(event, 
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await Lion.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(Lion, "Error Occurred\n {}".format(input_str))
    else:
        await eod(Lion, f"Syntax `{hl}rename file.name` as reply to a Telegram media")


@bot.on(Lion_cmd(pattern="rnupload (.*)", outgoing=True))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await Lion.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                    ms_one, ms_two
                )
            )
        else:
        ...
        await Lion.edit("Syntax // `{}rnupload file.name` as reply to a Telegram media".format(hl))


@bot.on(Lion_cmd(pattern="rnsupload (.*)", outgoing=True))
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await bot.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by [Lion-X](https://t.me/LIONX_official_chat)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
            ...
            await eod(Lion, "File Not Found {}".format(input_str))
    else:
        await Lion.edit(
