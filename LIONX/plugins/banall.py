...
@bot.on(Lion_cmd(pattern=r"kickall ?(.*)"))
...
async def _(event):
    ...
    Lion = await eor(event, "**Bleck Magik Started...**")
...
@bot.on(Lion_cmd(pattern=r"banall ?(.*)"))
...
async def _(event):
    ...
    Lion = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@bot.on(Lion_cmd(pattern=r"unbanall ?(.*)"))
...
async def _(event):
    ...
    q = 0
    r = 0
    Lion = await edit_or_reply(event, "Searching Participant Lists.")
    async for i in bot.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(Lion, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await Lion.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
