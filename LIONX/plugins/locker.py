...
@bot.on(Lion_cmd(pattern=r"lock ?(.*)", outgoing=True))
...
async def locks(event):
    ...
    try:
        ...
        await bot.send_file(
            event.chat_id,
            restlo,
            caption=f"{Lion_mention} Locked `{what}` \n__Cause its Rest Time !!__",
            )
...
@bot.on(Lion_cmd(pattern="unlock ?(.*)", outgoing=True))
    ...
    try:
        ...
        if Config.ABUSE == "ON":
            ...
            await bot.send_file(
                event.chat_id,
                shuru,
                caption=f"**{Lion_mention} unlocked** `{what}`",
            )
        else:
            await eor(event, f"{Lion_mention} Unlocked `{what}` \n__Now Start Chit Chat !!__")
    except BaseException as e:
        await eod(event, f"`Do I have proper rights for that ??`\n**Error:** {str(e)}")
        return

@bot.on(Lion_cmd(pattern="ltype$"))
