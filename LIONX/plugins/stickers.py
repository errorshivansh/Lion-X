...
LIONX = Config.STICKER_PACKNAME


@bot.on(Lion_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            Lion = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_LIONX_{pack}"
        packnick = (
            f"{LIONX} Vol.{pack}"
            if LIONX
            else f"@{user.username}'s Lion-X Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await Lion.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(Lion_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern=r"editst ?(.*)", outgoing=True))
    ...
    if Lion == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{Lion}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@Lionfonts")
...
@bot.on(Lion_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
