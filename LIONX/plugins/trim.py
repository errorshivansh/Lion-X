...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./trim/LIONX.media.ffmpeg"
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            Lionevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, Lionevent, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    ...
    Lionevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(
                Lionevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, Lionevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(Lionevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(Lionevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(Lionevent, f"`Completed Process in {ms} seconds`", 3)


@bot.on(Lion_cmd(pattern="atrim"))
...
@bot.on(Lion_cmd(pattern="tclear$"))
