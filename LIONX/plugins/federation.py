...
@bot.on(Lion_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lion_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {Lion_input}")
...
@bot.on(Lion_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {Lion_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(Lion_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lion = await eor(event, "`Collecting fstat....`")
    thumb = Lion_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await Lion.edit("`Please Unblock` @MissRose_Bot")


@bot.on(Lion_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lion = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
