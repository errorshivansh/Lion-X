...
@bot.on(Lion_cmd("kickme", outgoing=True))
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("😪 **KThnxBye** See u all in Lion!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@bot.on(Lion_cmd(pattern=r"config"))
...
@bot.on(Lion_cmd(pattern="schd ?(.*)", outgoing=True))
