...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Lionevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(Lionevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
@bot.on(Lion_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(Lion_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lionevent = await eor(event, "...")
    ...
    await Lionevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await Lionevent.delete()


@bot.on(Lion_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    Lion = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await Lion.edit(str(a))
    else:
        await eod(Lion, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(Lion_cmd(pattern="currency (.*)"))
...
@bot.on(Lion_cmd(pattern="currencies$"))
...
@bot.on(Lion_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Lionevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await Lionevent.edit(output_str, link_preview=True)
    else:
        await eod(Lionevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(Lion_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for Lion-X", ".currency 10 usd inr"
).add_command(
