...
@bot.on(Lion_cmd(pattern="bigspam"))
@bot.on(sudo_cmd(pattern="bigspam", allow_sudo=True))
async def bigspam(Lion):
    if not Lion.text[0].isalpha() and Lion.text[0] not in ("/", "#", "@", "!"):
        Lion_msg = Lion.text
        LIONX_count = int(Lion_msg[9:13])
        Lion_spam = str(Lion.text[13:])
        for i in range(1, LIONX_count):
            await Lion.respond(Lion_spam)
        await Lion.delete()
        await Lion.client.send_message(
                lg_id, f"#BIGSPAM \n\nBigspammed  `{Lion_count}`  messages !!"
        )


@bot.on(Lion_cmd("dspam (.*)"))
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what LIONX iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add_info(
  "Spammers Commands"
).add_warning(
