...
@bot.on(Lion_cmd(pattern=r"trc", outgoing=True))
@bot.on(sudo_cmd(pattern=r"trc", allow_sudo=True))
async def _(Lion):
    if Lion.fwd_from:
        return
    await edit_or_reply(Lion, "**All The Language Codes Can Be Found** \n ⚡ [Here](https://telegra.ph/SfMæisér--𐌷𐌴ࠋࠋ𐌱𐍈𐌸-𐌾𐌰𐍀𐌾-06-04) ⚡", link_preview=False)


@bot.on(Lion_cmd(pattern="voice (.*)"))
