...
@bot.on(Lion_cmd(pattern="ocrlang", outgoing=True))
...
@bot.on(Lion_cmd(pattern=r"ocr (.*)", outgoing=True))
...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(Lion, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), Lion_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await Lion.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await Lion.edit(ParsedText)
