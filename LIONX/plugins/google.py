...
@bot.on(Lion_cmd(pattern="wikipedia (.*)"))
...
@bot.on(Lion_cmd(pattern="watch (.*)"))
@bot.on(sudo_cmd(pattern="watch (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    query = event.pattern_match.group(1)
    Lion = await eor(event, "Finding Sites...")
...
@bot.on(Lion_cmd(pattern="img (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="img (.*)", allow_sudo=True))
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await eod(event, "`Give something to search...`")
    Lion = await eor(event, f"Searching for  `{sim}`...")
    if "-" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@bot.on(Lion_cmd(pattern="reverse"))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        Lion = await eor(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await Lion.edit("Found Google Result. Processing results...")
        headers = {
...
More Info: Open this <a href="{the_location}">Link</a> in {ms} seconds""".format(
    ...
    await Lion.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)


@bot.on(Lion_cmd(pattern="gps ?(.*)"))
