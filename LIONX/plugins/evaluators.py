...
async def _(event):
    ...
    if exc:
        ...
        evaluation = "Success"
    final_output = f"•  Eval : \n`{cmd}` \n\n•  Result : \n`{evaluation}` \n"
    await eor(Lionevent, "**Eval Command Executed. Check out LOGGER for result.**")
    await event.client.send_message(
        lg_id,
...
@bot.on(Lion_cmd(pattern="bash ?(.*)", outgoing=True))
