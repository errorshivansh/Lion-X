...
@bot.on(Lion_cmd(pattern=r"inviteall ?(.*)"))
...
async def get_users(event):
    ...
    if he_ll == "@LionXSupport":
        return await Lion.edit("Restricted to invite users from there.")
    elif he_ll == "@LionXSupport":
        return await Lion.edit("Restricted to invite users from there.")
    elif he_ll == "@LionXSupport":
        return await Lion.edit("Restricted to invite users from there.")
    crazy = await get_chatinfo(event)
    ...
    async for user in event.client.iter_participants(crazy.full_chat.id):
        try:
