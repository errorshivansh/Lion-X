...
@bot.on(Lion_cmd(pattern="extdl$", outgoing=True))
...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await bot.get_messages(chat, ids=mxo), "LIONX/plugins/"
        )
...
@bot.on(Lion_cmd(pattern=r"installall (.*)"))
@bot.on(sudo_cmd(pattern=r"installall (.*)", allow_sudo=True))
async def install(event):
    if event.fwd_from:
        return
    chat = event.pattern_match.group(1)
    Lion = await eor(event, f"Starting To Install Plugins From {chat} !!"
    )
    documentss = await bot.get_messages(chat, None, filter=InputMessagesFilterDocument)
    total = int(documentss.total)
    total_doxx = range(0, total)
