...
@bot.on(Lion_cmd(pattern="lyrics(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="lyrics(?: |$)(.*)", allow_sudo=True))
async def nope(crazy):
    Lion = crazy.pattern_match.group(1)
    await eor(crazy, f"Searching lyrics for  `{Lion}` ...")
    if not Lion:
        if crazy.is_reply:
            (await crazy.get_reply_message()).message
        else:
            await eod(crazy, "Give song name to get lyrics...")
            return

    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(Lion))}")
...
@bot.on(Lion_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(Lion_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
