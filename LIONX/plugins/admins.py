...
from . import *
from LIONX.sql.mute_sql import is_muted, mute, unmute
...
@bot.on(Lion_cmd(pattern="setgpic$"))
...
@bot.on(Lion_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    Lionevent = await eor(promt, "`Promoting User...`")
    ...
    try:
        ...
        await Lionevent.edit(f"**🔥 Promoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
...
@bot.on(Lion_cmd(pattern="demote(?: |$)(.*)"))
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    Lionevent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await Lionevent.edit(NO_PERM)
        return
    await Lionevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
@bot.on(Lion_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(Lion):
    if Lion.is_private:
        await eor(Lion, "**Enough of your bullshit  !!**")
        await sleep(2)
        await Lion.get_reply_message()
        replied_user = await Lion.client(GetFullUserRequest(Lion.chat_id))
        if is_muted(Lion.chat_id, Lion.chat_id):
            return await Lion.edit(
                "Nigga is already muted here 🥴"
            )
        if Lion.chat_id == ForGo10God:
            return await eod(Lion, "Nashe me hai kya lawde 🥴")
        try:
            mute(Lion.chat_id, Lion.chat_id)
        except Exception as e:
            await eor(Lion, f"**Error **\n`{str(e)}`")
        else:
            await eor(Lion, "**Chup Reh Lawde 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        Lionevent = await eor(Lion, "`Muting...`")
        input_str = Lion.pattern_match.group(1)
        chat = await Lion.get_chat()
        if Lion.reply_to_msg_id:
            userid = (await Lion.get_reply_message()).sender_id
            name = (await Lion.client.get_entity(userid)).first_name
        elif input_str:
            if input_str.isdigit():
                try:
                    ...
                    return await Lionevent.edit(str(ve))
            else:
                userid = (await Lion.client.get_entity(input_str)).id
                name = (await Lion.client.get_entity(userid)).first_name
        else:
            return await eod(Lionevent, "I Need a user to mute!!", 5)
        if userid == ForGo10God:
            return await eod(Lionevent, "Nashe me hai kya lawde", 5)
        if str(userid) in DEVLIST:
            return await eod(Lionevent, "**Error Muting God**", 7)
        try:
            await Lion.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
...
@bot.on(Lion_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        Lionevent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(Lionevent, "I need a user to unmute!!", 3)
        try:
...
@bot.on(Lion_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    Lionevent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await Lionevent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await Lionevent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await Lionevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await Lionevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await Lionevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    Lionevent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await Lionevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await Lionevent.edit("Invalid UserId!! Please Recheck it!!")


@bot.on(Lion_cmd(pattern="pin($| (.*))"))
...
async def kick(usr):
    ...
    Lionevent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await Lionevent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        ...
        await Lionevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(Lion_cmd(pattern=f"zombies ?(.*)"))
