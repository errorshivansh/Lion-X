...
LIONX_NAME = Config.YOUR_NAME or "ʟɨօռ-Ӽ"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await eor(event, 
                   "✓ **File uploaded to [telegraph](https://telegra.ph{})** \n✓ **Time Taken :-** `{}` secs \n✓ **By :- {}**".format(
                        media_urls[0], (ms + ms_two), Lion_mention,
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            crazyboy = f"https://telegra.ph/{response['path']}"
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({crazyboy}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :**  {Lion_mention}", link_preview=True)
    else:
        await eod(event, 
