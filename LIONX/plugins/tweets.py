...
@bot.on(Lion_cmd(pattern=r"tweet(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="tweet(?: |$)(.*)", allow_sudo=True))
async def nope(crazy):
    Lion = crazy.pattern_match.group(1)
    okvai = await eor(crazy, "Trying to tweet for you...")
    if not Lion:
        if crazy.is_reply:
            (await crazy.get_reply_message()).message
        else:
            await eod(crazy, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(Lion))}")
    await tweeter[0].click(
        crazy.chat_id,
        reply_to=crazy.reply_to_msg_id,
        silent=True if crazy.is_reply else False,
        hide_via=True,
    )
    await crazy.delete()


@bot.on(Lion_cmd(pattern=r"trump(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"modi(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"dani(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"pappu(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"sunny(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"johhny(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"gandhi(?: |$)(.*)"))
...
@bot.on(Lion_cmd(pattern=r"kanna(?: |$)(.*)"))
