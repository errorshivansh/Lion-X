...
@bot.on(Lion_cmd(pattern=r"tsearch ?(.*)"))
...
async def tor_search(event):
    ...
    Lion = await eor(event, f"Searching for  `{search_str}` .....")
    if " " in search_str:
    ...
    for div in source.find_all("div", {"class": "grey_bar3 back_none"}):
        ...
        counter += 1
    if not urls:
        await eod(Lion, "Either the Keyword was restricted or not found..", 7)
        return
    for url in urls:
    ...
    while counter != len(titles):
        ...
        counter += 1
    await Lion.edit(msg, link_preview=False)


@bot.on(Lion_cmd(pattern=r"movie (torrentz2\.eu|idop\.se) (.*)"))
