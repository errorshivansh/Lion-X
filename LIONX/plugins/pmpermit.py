...
from . import *
from LIONX.sql import pmpermit_sql as pm_sql
...
LIONX_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
LIONX_FIRST = (
    "**🔥 ʟɨօռ-Ӽ Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**Please Choose Why You Are Here!!**".format(Lion_mention, CSTM_PMP)
)

@bot.on(Lion_cmd(pattern="block$"))
...
if PM_ON_OFF != "DISABLE":
    ...
    @bot.on(Lion_cmd(pattern="(a|approve|allow)$"))
    ...
    @bot.on(Lion_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @bot.on(Lion_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(LIONX_ZERO)
            await asyncio.sleep(3)
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by ʟɨօռ-Ӽ"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
