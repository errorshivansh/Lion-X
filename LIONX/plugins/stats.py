...
async def stats(
    ...
    if event.fwd_from:
        return
    Lion = await edit_or_reply(event, "`Collecting stats...`")
    ...
    response += (
        f"📌 **From The DataBase Of** :- {Lion_channel}"
    )
    await Lion.edit(response)
...
def inline_mention(user):
    full_name = user_full_name(user) or "ʟɨօռ"
