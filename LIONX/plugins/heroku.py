...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "Lion-X")


@bot.on(Lion_cmd(pattern="restart$"))
@bot.on(sudo_cmd(pattern="restart$", allow_sudo=True))
async def re(Lion):
    if Lion.fwd_from:
        return
    event = await eor(Lion, "Restarting Dynos ...")
    if HEROKU_API_KEY:
        await restart(event)
    else:
        await event.edit("Please Set Your `HEROKU_API_KEY` to restart ʟɨօռ-Ӽ")


@bot.on(Lion_cmd(pattern="shutdown$"))
@bot.on(sudo_cmd(pattern="shutdown$", allow_sudo=True))
async def down(Lion):
    if Lion.fwd_from:
        return
    await eor(Lion, "**[ ! ]** Turning off ʟɨօռ-Ӽ Dynos... Manually turn me on later ಠ_ಠ")
...
@bot.on(Lion_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True))
@bot.on(sudo_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(Lion):
    if Lion.fwd_from:
        return
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(Lion, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = Lion.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(Lion, "Getting Variable Info...")
        await asyncio.sleep(1.5)
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            variable = Lion.pattern_match.group(2).split()[0]
            if variable in ("LIONX_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await bot.send_file(Lion.chat_id, cjb, caption=cap)
                    await event.delete()
        ...
        event = await eor(Lion, "Setting Heroku Variable...")
        variable = Lion.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = Lion.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = Lion.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(Lion, "Getting info to delete Variable")
        try:
            variable = Lion.pattern_match.group(2).split()[0]
        except IndexError:
...
@bot.on(Lion_cmd(pattern="usage(?: |$)", outgoing=True))
@bot.on(sudo_cmd(pattern="usage(?: |$)", allow_sudo=True))
async def dyno_usage(Lion):
    if Lion.fwd_from:
        return
    event = await edit_or_reply(Lion, "`Processing...`")
    useragent = (
...
@bot.on(Lion_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {Lion_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {Lion_grp} for help.", link_preview=False)
    event = await eor(dyno, "Downloading Logs...")
    with open("LIONX-logs.txt", "w") as log:
    ...
    await event.delete()
    return os.remove("LIONX-logs.txt")
    
  # Lion_data = app.get_log()
  # await eor(
  #     dyno, Lion_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {Lion_mention}\n\n🚀** Pasted**  "
  # )
"""
    key = (
        requests.post("https://nekobin.com/api/documents", json={"content": Lion_data})
        .json()
        .get("result")
        .get("key")
    )
    Lion_url = f"https://nekobin.com/{key}"
    url_raw = f"https://nekobin.com/raw/{key}"
    foutput = f"**🗒️ Heroku Logs of 💯 lines. 🗒️** \n\n📍 [Nekobin]({Lion_url}) & [Raw]({url_raw}) 📍\n\n🌟 **Bot Of :**  {Lion_mention}"
"""
