...
@bot.on(Lion_cmd(pattern="(rmbg|srmbg) ?(.*)"))
...
async def remove_background(event):
    ...
    remove_bg_image = "Lion-X.png"
    if "image" in contentType:
        with open("Lion-X.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_or_reply(Lionevent, f"`{response.content.decode('UTF-8')}`")
        return
    if cmd == "srmbg":
        file = convert_tosticker(remove_bg_image, filename="Lion-X.webp")
