...
@bot.on(Lion_cmd(pattern="anilist (.*)"))
...
@bot.on(Lion_cmd(pattern="anime(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="anime(?: |$)(.*)", allow_sudo=True))
async def nope(hel_):
    Lion = hel_.pattern_match.group(1)
    if not Lion:
        if hel_.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(Lion))}")
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(Lion))}")
...
@bot.on(Lion_cmd(pattern="character(?: |$)(.*)"))
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(Lion))}")
