...
from LIONX.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "✅ Successfully updated ʟɨօռ-Ӽ!\n\nBot is restarting please wait for a minute."
    )
    args = [sys.executable, "-m", "LIONX"]
    os.execle(sys.executable, *args, os.environ)
    return


@bot.on(Lion_cmd(outgoing=True, pattern=r"update(| now)$"))
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **ʟɨօռ-Ӽ** !!")

    if force_update:
        await event.edit(
            "`Force-Updating ʟɨօռ-Ӽ. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your ʟɨօռ-Ӽ Is UpToDate**\n\n**Version :**  __{Lion_ver}__\n**Oɯɳҽɾ :**  {Lion_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@bot.on(Lion_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    off_repo = "https://github.com/TeamLionX/Lion-X"
    os.chdir("/app")
    git_Lion = f"rm -rf .git"
    try:
        await runner.runcmd(git_Lion)
    except BaseException:
        pass
    try:
        txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**ʟɨօռ-Ӽ Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your ʟɨօռ-Ӽ. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your ʟɨօռ-Ӽ. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "ʟɨօռ-Ӽ Updater."
).add_warning(
  "✅ Harmless Module."
