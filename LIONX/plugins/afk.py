...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=Lionpic)
...
@bot.on(Lion_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    crazyop = await event.get_reply_message()
    global USER_AFK  # pylint:disable=E0602
    global afk_time  # pylint:disable=E0602
    global last_afk_message  # pylint:disable=E0602
    global afk_start
    ...
    Lionpic = await event.client.download_media(crazyop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {Lionpic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"**I'm going afk🚶** \n\n**Because :** {reason}", file=Lionpic
            )
        else:
            await bot.send_message(
                event.chat_id, f"**I am Going afk!**🚶", file=Lionpic)
        ...
        try:
            if reason:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**\nReason  `{reason}`",file=Lionpic
                 )
            else:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**",file=Lionpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
...
@bot.on(Lion_cmd(pattern=r"night ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
